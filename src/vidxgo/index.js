...
if (!IS_SERVER) {
  var extractVidxGo = require('../extractors/vidxgo').extractVidxGo;
  var checkQualityFromPlaylist = require('../quality_helper.js').checkQualityFromPlaylist;
  var getQualityFromName = function(q) {
    if (!q) return 'Unknown';
    ...
    if (m) { var r = parseInt(m[1]); if (r >= 2160) return '4K'; if (r >= 1440) return '1440p'; if (r >= 1080) return '1080p'; if (r >= 720) return '720p'; if (r >= 480) return '480p'; if (r >= 360) return '360p'; return '240p'; }
    return 'Unknown';
  };
  async function resolveImdbId(id) {
    var imdbId = String(id || '').replace('tmdb:', '');
    if (/^tt\d+$/.test(imdbId)) return imdbId;
    if (/^\d+$/.test(imdbId)) {
      var key = '68e094699525b18a70bab2f86b1fa706';
      try {
        var r = await fetch('https://api.themoviedb.org/3/movie/' + imdbId + '?api_key=' + key);
        if (r.ok) { var d = await r.json(); if (d.imdb_id) return d.imdb_id; }
        var r2 = await fetch('https://api.themoviedb.org/3/movie/' + imdbId + '/external_ids?api_key=' + key);
        if (r2.ok) { var d2 = await r2.json(); if (d2.imdb_id) return d2.imdb_id; }
        var r3 = await fetch('https://api.themoviedb.org/3/tv/' + imdbId + '?api_key=' + key);
        if (r3.ok) { var d3 = await r3.json(); if (d3.imdb_id) return d3.imdb_id; }
        var r4 = await fetch('https://api.themoviedb.org/3/tv/' + imdbId + '/external_ids?api_key=' + key);
        if (r4.ok) { var d4 = await r4.json(); if (d4.imdb_id) return d4.imdb_id; }
      } catch (e) {}
    }
    return null;
  }
  ...
  module.exports = {
    getStreams: async (id, type, season, episode) => {
      try {
        ...
        var imdbId = await resolveImdbId(id);
        if (!imdbId) return [];
        ...
        if (proxyUrl && proxyPassword) {
          ...
          return [formatStream({
            url: targetUrl, name: 'VidxGo', title: displayName, quality: '1080p',
            language: 'Italian', size: 'proxied', type: 'direct', headers: null,
            behaviorHints: { proxyHeaders: null, headers: null }
          }, 'VidxGo')].filter(Boolean);
        }

        var extracted = await extractVidxGo(vidxgoUrl);
        if (!extracted || !extracted.url) return [];
        var quality = 'HD';
        var detected = await checkQualityFromPlaylist(extracted.url, extracted.headers);
        if (detected) quality = detected;
        return [formatStream({
          url: extracted.url, headers: extracted.headers, name: 'VidxGo',
          title: displayName, quality: getQualityFromName(quality), type: 'direct',
          language: 'Italian'
        }, 'VidxGo')].filter(Boolean);
      } catch (e) {
